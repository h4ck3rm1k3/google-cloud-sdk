...
"$CLOUDSDK_PYTHON" $CLOUDSDK_PYTHON_ARGS "${CLOUDSDK_ROOT_DIR}/bin/bootstrapping/prerun.py" --command-name=appcfg-java --component-id=gae-java --check-credentials --check-updates || exit

CREDENTIALS_PATH=$("$CLOUDSDK_PYTHON" $CLOUDSDK_PYTHON_ARGS "${CLOUDSDK_ROOT_DIR}/bin/bootstrapping/print_env_info.py" gae_java_path) || exit

"${CLOUDSDK_ROOT_DIR}/platform/appengine-java-sdk/bin/appcfg.sh" --oauth2 --oauth2_config_file="$CREDENTIALS_PATH" "$@"
