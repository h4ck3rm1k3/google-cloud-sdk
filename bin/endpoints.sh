...
"$CLOUDSDK_PYTHON" $CLOUDSDK_PYTHON_ARGS "${CLOUDSDK_ROOT_DIR}/bin/bootstrapping/prerun.py" --command-name=endpoints-java --component-id="gae-java" --check-updates || exit

"${CLOUDSDK_ROOT_DIR}/platform/appengine-java-sdk/bin/endpoints.sh" "$@"
