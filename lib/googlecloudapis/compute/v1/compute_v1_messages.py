...
class AttachedDiskInitializeParams(messages.Message):
  """Initialization parameters for the new disk (Mutually exclusive with
  'source', can currently only be specified on the boot disk).
...
class BackendService(messages.Message):
  ...
  protocol = messages.EnumField('ProtocolValueValuesEnum', 10)
...
class DiskType(messages.Message):
  ...
  description = messages.StringField(3)
  id = messages.IntegerField(4, variant=messages.Variant.UINT64)
  kind = messages.StringField(5, default=u'compute#diskType')
  name = messages.StringField(6)
  selfLink = messages.StringField(7)
  validDiskSize = messages.StringField(8)
